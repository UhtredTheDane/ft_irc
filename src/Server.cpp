...
Server::Server(int port, std::string password)
{
	...
	requests_ptr[3] = &Server::kick_request;
	requests_ptr[4] = &Server::privmsg_request;
	requests_ptr[5] = &Server::part_request;
}
...
void Server::pong_request(User* user)
{
	msg.pong_msg(user);
}

void Server::join_request(User* user)
{
		try
		{	
...
void Server::mode_request(User* user)
{
		try
		{
...
void Server::kick_request(User* user)
{
...
void Server::privmsg_request(User* user)
{
	try
		{
			if(split_line[1][0] == '#')
...
void Server::part_request(User* user)
{
		try
		{
			Channel *current_chan = channels.at(split_line[1]); 
